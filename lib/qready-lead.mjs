...
export async function manejarQreadyLead(b, env, enviar = enviarAviso) {
  ...
  try {
    ...
    // console.error, no silencio: un 500 sin rastro es indistinguible de cualquier
    // otro 500, y "wrangler tail" no muestra nada si el handler no deja nada escrito.
    console.error("qready-lead: fallo el INSERT:", (e && e.message) || e);
    if (!/no such table/i.test(String((e && e.message) || e))) {
      return { status: 500, cuerpo: { ok: false, error: "no se pudo registrar" } };
    }
    try {
      // `env.DB.exec()` corta por salto de linea y ejecuta cada linea como sentencia
      // aparte — con un CREATE TABLE multilinea manda solo "CREATE TABLE IF NOT EXISTS
      // qready_leads (" y explota con "incomplete input". `.prepare().run()` no tiene
      // ese problema: una sola sentencia, se manda entera. Lo vi en produccion con
      // wrangler tail, no lo supuse.
      await env.DB.prepare(CREAR_TABLA).run();
      ins = await insertar(env, b);
    } catch (e2) {
      console.error("qready-lead: fallo crear tabla o reintentar:", (e2 && e2.message) || e2);
      return { status: 500, cuerpo: { ok: false, error: "no se pudo registrar" } };
    }
  }
  ...
  try {
    ...
    console.error("qready-lead: fallo el aviso por correo:", emailError);
  }
  try {
...
/**
 * Una linea, sin CR ni LF. Toda cabecera de correo que lleve un campo del formulario
 * pasa por aca: sin esto, un lead con `\r\nBcc: otro@dominio.com` en el email o el
 * telefono inyecta cabeceras nuevas en el correo que le llega a Nicholas — el mismo
 * problema que un `\n` en un valor de SQL, aplicado a texto plano en vez de a una
 * consulta. El cuerpo del mensaje no lo necesita (ahi un salto de linea es legitimo);
 * esto es solo para lo que se interpola en Subject/From/Reply-To.
 */
const unaLinea = s => String(s || "").replace(/[\r\n]+/g, " ").trim();

/**
 * Arma el correo. Separado de `enviarAviso` a proposito: esta parte es texto puro y se
 * puede probar en Node; `enviarAviso` importa "cloudflare:email", que solo existe en el
 * runtime de Workers y no se puede ejercer fuera de el.
 */
export function construirCorreo(l) {
  const legal = unaLinea(l.legal);
  const email = unaLinea(l.email);
  const cuerpo = [
  ...
  // Reply-To es el propio pedido de este encargo: que Nicholas responda al prospecto
  // con un clic desde Gmail. Va SIEMPRE que el email paso la validacion de
  // manejarQreadyLead (formato valido), asi que aca no hace falta re-validarlo — pero
  // igual pasa por unaLinea() porque una cabecera nunca confia en el origen del dato.
  const raw = `From: leads@rosettaquantum.com\r\nTo: hello@rosettaquantum.com\r\n` +
    `Reply-To: ${email}\r\nSubject: Q-Ready lead: ${legal}\r\n` +
