...
const PALETAS = {
  ...
  // PAPEL — repintado para /pilots (commit 13). El sistema visual de la marca es papel
  // e tinta: la paleta oscura de esta pieza nacio para el hero viejo. Aqui NO hay niebla,
  // NO hay bloom y NO hay mezcla aditiva: sobre papel, lo aditivo se va a blanco y el
  // punto desaparece justo donde tiene que verse.
  // Y los dos marcadores llevan colores DISTINTOS a proposito: los residuos fuente en
  // teal y los sitios predichos en rojo. Con el mismo color, la pieza estaria afirmando
  // que la prediccion coincide con la verdad — que es justo lo que este experimento
  // midio que NO pasa.
  papel: {
    fondoAlto: 0xF4F5F1, fondoBajo: 0xECEEE8, niebla: null,
    rampa: [0x16181B, 0x5A5D58, 0x0F8B7E],   // tinta · tinta-60 · teal
    acento: 0x0F8B7E, traza: 0xD9DBD6,
    anillo: 0x0F8B7E,                         // residuos fuente (lo medido)
    sitio: 0xB4432F,                          // sitios predichos (lo afirmado)
    mate: true,
  },
  oficial: {
...
export function montar(contenedor, { pdb, target, paleta = 'oficial', hero = false, encuadre }) {
  ...
  const PRESETS = {
    ...
    // se veía "como salvapantallas" en el hero, reportado al mirarlo girar. Se
    ...
    columna:  {
      ...
      distancia: (radio, aspecto) => {
        ...
        // 1.22 dejaba "mucho aire" (reporte de direccion, mirando la columna real).
  ...
  escena.fog = LOOK.niebla == null ? null : new THREE.FogExp2(LOOK.niebla, 0.006);
  escena.add(new THREE.AmbientLight(0xffffff, LOOK.mate ? 1.05 : 0.75));
  // Luces neutras en la version de papel: la azul y la rosa del hero viejo tenian la
  // fuerza suficiente para teñir la tinta, y la tinta teñida deja de ser el color de la
  // marca sin que nadie lo note.
  const key = new THREE.DirectionalLight(LOOK.mate ? 0xffffff : 0xa8d8ff, LOOK.mate ? 1.1 : 2.2);
  key.position.set(1, 1.1, 1.2); escena.add(key);
  const fill = new THREE.DirectionalLight(LOOK.mate ? 0xffffff : 0xff5fd0, LOOK.mate ? 0.45 : 1.25);
  fill.position.set(-1.3, -0.5, -0.9); escena.add(fill);
  ...
  function construir(d) {
    ...
    malla = new THREE.InstancedMesh(new THREE.SphereGeometry(1, 16, 12),
      new THREE.MeshStandardMaterial(LOOK.mate
        ? { metalness: 0, roughness: 0.88, emissive: 0x000000, emissiveIntensity: 0 }
        : { metalness: 0.25, roughness: 0.22, emissive: 0x0a1428, emissiveIntensity: 0.9 }), d.n);
    escena.add(malla);
    ...
    d.src.forEach(i => {
      const a = new THREE.Mesh(new THREE.TorusGeometry(1.15, 0.10, 8, 22),
        new THREE.MeshBasicMaterial({ color: LOOK.anillo || LOOK.acento, transparent: true,
          opacity: LOOK.mate ? 0.95 : 0.8,
          blending: LOOK.mate ? THREE.NormalBlending : THREE.AdditiveBlending,
          depthWrite: !!LOOK.mate }));
    ...
    d.sites.forEach(s => {
      ...
      const b = new THREE.Mesh(new THREE.IcosahedronGeometry(r, 1),
        new THREE.MeshBasicMaterial({ color: LOOK.sitio || LOOK.acento, wireframe: true, transparent: true, opacity: LOOK.mate ? 0.75 : 0.5 }));
    ...
    if (escena.fog) escena.fog.density = 0.9/(radio*8);
  ...
  function pintar(dt61) {
    ...
    for (let i = 0; i < prob.length; i++) {
      ...
      // 28-ago, tras ver direccion la pieza en movimiento: "es como fuegos artificiales,
